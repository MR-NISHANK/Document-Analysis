...
class MyThread_1 implements Runnable{
    ...
    public void run() {
        ...
        File pdf = new File("D:\\pdf");
        File[] PDFFiles = pdf.listFiles();
        File docx = new File("D:\\doc");
        File[] DocxFiles = docx.listFiles();
        File Txt = new File("D:\\text");
        File Search = new File("D:\\Search");
        ...
        if(count==1){
            ...
            for (int i = 0; i < PDFFiles.length; i++) {
                if (PDFFiles[i].isFile()) {
                    ...
                    try {
                        new Thread(con[1].newInstance(Txt_path,synchronousQueue)).start();
        ...
        if(count==2){
            ...
            for (int i = 0; i < PDFFiles.length; i++) {
                if (PDFFiles[i].isFile()) {
                    ...
                    Constructor<? extends Thread> co[] = (Constructor<? extends Thread>[]) d.getConstructors();
                    try {
                        new Thread(co[0].newInstance(Search_path,Key,synchronousQueue));
                    } catch (InstantiationException e) {
                        e.printStackTrace();
                    } catch (IllegalAccessException e) {
                        e.printStackTrace();
                    ...
                    Constructor<?extends Thread> cons[] = (Constructor<? extends Thread>[]) b.getConstructors();
                    try {
                        new Thread(cons[0].newInstance(Pdf_path,Docx_path,synchronousQueue));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
